#include "../include/light_sensor.hpp"
...
res light_sensor::get_status() {
	return Ok(m_status);
}

res light_sensor::initialize() {
    esp_err_t err;
    ...
    err = i2cdev_init();
    if (err) {
        RETURN_ON_ERR_MSG(err, "Failed to init i2c device: ");
    }

    err = veml7700_init_desc(&m_dev, I2C_NUM_0, static_cast<gpio_num_t>(SDA_GPIO_NUM), static_cast<gpio_num_t>(SCL_GPIO_NUM));
    if (err) {
        RETURN_ON_ERR_MSG(err, "Failed to init light sensor i2c communication: ");
    }

    err = veml7700_probe(&m_dev);
    if (err) {
        RETURN_ON_ERR_MSG(err, "Failed to probe light sensor: ");
    }

    m_conf.shutdown = 0;
    err = veml7700_set_config(&m_dev, &m_conf);
    if (err) {
        RETURN_ON_ERR_MSG(err, "Failed to set light sensor config: ");
    }
...
Result<uint32_t, etl::string<128>> light_sensor::read_light_level() {
    uint32_t value_lux;
    if (m_status != RUNNING) {
        return Err(etl::string<128>("not initialized"));
    }

    esp_err_t err = veml7700_get_ambient_light(&m_dev, &m_conf, &value_lux);
    if (err) {
        RETURN_ON_ERR_MSG(err, "Failed to read ambient light value: ");
    }
