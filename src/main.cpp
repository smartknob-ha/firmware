...
#define DC_PIN_NUM 16

// for the example
#include "gui.hpp"

lv_color_t EXT_RAM_BSS_ATTR vram_0[240 * 50];
lv_color_t EXT_RAM_BSS_ATTR vram_1[240 * 50];
...
extern "C" void app_main(void) {
	...
	gpio_num_t spi_mosi = GPIO_NUM_6;
	gpio_num_t spi_sclk = GPIO_NUM_5;
	gpio_num_t spi_cs = GPIO_NUM_15;
	gpio_num_t lcd_reset = GPIO_NUM_4;
	gpio_num_t lcd_dc_pin = GPIO_NUM_16;
	gpio_num_t lcd_backlight = GPIO_NUM_7;
	size_t width = 240;
	size_t height = 240;
	...
	auto display = std::make_shared<espp::Display>(
		espp::Display::NonAllocatingConfig{
			.vram0 = vram_0,
			.vram1 = vram_1,
			.width = width,
			.height = height,
