...
[[noreturn]] void startSmartknob(void) {
    ...
    // StrainSensor strainSensor((gpio_num_t)CONFIG_STRAIN_SENSOR_DOUT_GPIO_NUM, (gpio_num_t)CONFIG_STRAIN_SENSOR_SCK_GPIO_NUM);
    StrainSensor strainSensor((gpio_num_t )6,  (gpio_num_t )5);
...
//    sdk::Manager::addComponent(ringLights);
    ...
    // sdk::Manager::addComponent(strainSensorB);
...
//            ESP_LOGI("main", "raw encoder radians: %lf", magneticEncoder.getDevice()->get()->get_radians());
            ...
            ESP_LOGI("main", "strain state / value: %s / %ld", StateString.c_str(), strainSensor.readAverageStrainLevel(10).value());
            // ESP_LOGI("main", "strain percentage: %d", strainLevel.percentage);

//            ESP_LOGI("main", "raw strain A level: %ld", (strainSensorA.readAverageStrainLevel(10).value() + 1210900) / 100);
            // ESP_LOGI("main", "raw strain level: %ld", 877300 + strainSensor.readAverageStrainLevel(10).value());

            // lv_label_set_text(label1, StateString.c_str());

            if (strainLevel.level == StrainSensor::HARD_PRESS) {
